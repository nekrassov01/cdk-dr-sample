...
export class Service extends Construct {
  ...
  readonly nlb: cdk.aws_elasticloadbalancingv2.INetworkLoadBalancer;
  ...
  constructor(scope: Construct, id: string, props: ServiceProps) {
    ...
    asg.connections.allowFrom(alb, cdk.aws_ec2.Port.tcp(80), "Allow access to EC2 instance from ALB on port 80");

    // ALB HTTPS listener
    alb.addListener("ListenerHTTPS", {
    ...
    this.nlb = new cdk.aws_elasticloadbalancingv2.NetworkLoadBalancer(this, "NLB", {
      loadBalancerName: `${props.serviceName}-${props.area}-nlb`,
      vpc: props.vpc,
      vpcSubnets: props.publicSubnets,
      internetFacing: true,
      crossZoneEnabled: true,
    });
    this.nlb.node.addDependency(alb);
    ...
    cfnLoadBalancer.subnetMappings = props.vpc.publicSubnets.map((subnet, i) => {
      ...
      return {
        ...
        allocationId: cdk.Token.asString(eip.getAtt("AllocationId")),
      } as cdk.aws_elasticloadbalancingv2.CfnLoadBalancer.SubnetMappingProperty;
    });
    ...
    this.nlb.addListener("ListenerTCP443", {
      protocol: cdk.aws_elasticloadbalancingv2.Protocol.TCP,
      port: 443,
      defaultTargetGroups: [
        new cdk.aws_elasticloadbalancingv2.NetworkTargetGroup(this, "NLBTargetGroupTCP443", {
          targetGroupName: `${props.serviceName}-${props.area}-nlb-tg-443`,
          targetType: cdk.aws_elasticloadbalancingv2.TargetType.ALB,
          targets: [new cdk.aws_elasticloadbalancingv2_targets.AlbTarget(alb, 443)],
          protocol: cdk.aws_elasticloadbalancingv2.Protocol.TCP,
          port: 443,
          preserveClientIp: true,
          healthCheck: {
    ...
    // NLB TCP80 listerner
    this.nlb.addListener("ListenerTCP80", {
      protocol: cdk.aws_elasticloadbalancingv2.Protocol.TCP,
      port: 80,
      defaultTargetGroups: [
        new cdk.aws_elasticloadbalancingv2.NetworkTargetGroup(this, "NLBTargetGroupTCP80", {
          targetGroupName: `${props.serviceName}-${props.area}-nlb-tg-80`,
          targetType: cdk.aws_elasticloadbalancingv2.TargetType.ALB,
          targets: [new cdk.aws_elasticloadbalancingv2_targets.AlbTarget(alb, 80)],
          protocol: cdk.aws_elasticloadbalancingv2.Protocol.TCP,
          port: 80,
          preserveClientIp: true,
          healthCheck: {
